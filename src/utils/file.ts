...
async function downWxFile(url: string, filePath: WxPath) {
  await checkWxDir(filePath)
...
function cleanCacheFiles(prefix = URL_PREFIX, count = URL_MAX) {
  ...
  return new Promise<void>(resolve => {
    fsm.stat({
      ...
      success: ({ stats }) => {
        try {
          const statsArray = stats as WechatMiniprogram.FileStats[]
...
export async function loadUrl(url: string, prefix = URL_PREFIX) {
